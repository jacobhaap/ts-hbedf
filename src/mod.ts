...
type Hbedf = {
    (passphrase: string, rekeying: boolean, identity: string[], secret: string | null): Buffer;
    rng: (secret: string | Buffer, data: string, counter: number | null) => number;
    shuffle: (secret: string, rekeying: boolean, input: string[]) => string;
}
