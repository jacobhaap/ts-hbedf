...
Deno.test(`'hbedf.rng' produces a seeded pseudorandom number from an HMAC-SHA256`, () => {
    const key = Buffer.from(secret);
    const rng = hbedf.rng(key, secret, null);
    console.assert(!!rng && typeof rng === 'number', `'hbedf.rng' should produce a pseudorandom number from an HMAC`);
});

Deno.test(`'hbedf.shuffle' shuffles an array using the Fisher-Yates Shuffle Algorithm, with rekeying`, () => {
    const shuffle = hbedf.shuffle(passphrase, true, identity);
    console.assert(!!shuffle && typeof shuffle === 'string', `'hbedf.shuffle' should shuffle an array, with rekeying`);
});
